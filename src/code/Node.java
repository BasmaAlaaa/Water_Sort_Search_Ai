...
public class Node {
    ...
    // Constructor
    public Node(List<Bottle> state, Node parent, String action, int pathCost) {
        this.state = state;
        this.parent = parent;
        this.action = action;
    ...
    // Helper function to check if a single bottle is sorted (i.e., all layers are of the same color or empty)
    private boolean isBottleSorted(Bottle bottle) {
        if (bottle.isEmpty()) return true; // An empty bottle is considered sorted
    ...
    // Generate the child nodes by applying valid actions (pouring from one bottle to another)
        ...
        for (int i = 0; i < state.size(); i++) {
            for (int j = 0; j < state.size(); j++) {
                if (i != j && isValidAction(state.get(i), state.get(j))) {
                    ...
                    Node child = new Node(newState, this, action, this.pathCost + 1);
                    children.add(child);
                }
            }
        }
